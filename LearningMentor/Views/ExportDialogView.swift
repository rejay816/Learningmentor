...
struct ExportDialogView: View {
    let messages: [LMChatMessage]
    
    // 多选
    @Binding var selectedMessages: [LMChatMessage]
    
    var onDismiss: () -> Void
    var onExport: ([LMChatMessage], ExportFormat) -> Void
    ...
    init(messages: [LMChatMessage], selectedMessages: Binding<[LMChatMessage]>, onDismiss: @escaping () -> Void, onExport: @escaping ([LMChatMessage], ExportFormat) -> Void) {
    ...
    // 添加拖拽状态
    @State private var draggedMessage: LMChatMessage?
    ...
    private func parseToAnkiCardsForFrenchListening(messages: [LMChatMessage]) -> [FrenchListeningCard] {
    ...
    private func parseMultipleFrenchListeningCards(_ messages: [LMChatMessage]) -> [FrenchListeningCard] {
        let combinedContent = messages.map { $0.content }.joined(separator: "\n")
